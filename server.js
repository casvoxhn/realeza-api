...
app.post('/generate', async (req, res) => {
  try {
    ...
    console.error('⚠️ ERROR V15.0:', error.message);
    res.status(500).json({ success: false, error: error.message });
  }
});

app.get('/health', (req, res) => {
  res.json({ status: 'ok', version: 'V15.0', model: MODEL_ID, styleRefs: STYLE_REFS.length });
});

app.listen(PORT, () => {
  console.log(`🚀 V15.0 | Puerto ${PORT} | Modelo: ${MODEL_ID} | Refs: ${STYLE_REFS.length}`);
});
